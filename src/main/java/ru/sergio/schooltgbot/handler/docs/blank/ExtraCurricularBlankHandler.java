...
@Service
public class ExtraCurricularBlankHandler implements BotHandler {
    @Override
    public PartialBotApiMethod<Message> handle(Update update) {
        InputStream doc = getClass().getClassLoader().getResourceAsStream("img/docs/extra_curricular_blank.pdf");
        InputFile file = new InputFile(doc, "Заявление на зачисление в коллектив доп образования.pdf");
